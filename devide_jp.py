...
csv_path = os.path.join(dir_path, 'CSV_jp')
...
def main():
    ...
    for meeting_id in meeting_ids:
        ...
        # CSVファイルのパス
        csv_file = os.path.join(csv_path, f"{meeting_id}_jp.csv")
        dialogue_turns = DialogueTurn.from_csv(csv_file)
        
        # CSV_topicsディレクトリを作成（存在しない場合）
        csv_topics_path = os.path.join(dir_path, 'CSV_topics_jp')
        ...
        for i in range(len(topic_list)):
            ...
            output_csv_file = os.path.join(csv_topics_path, f"{meeting_id}-{current_topic}_jp.csv")
